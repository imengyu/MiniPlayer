...
bool CWmaDecoder::Open(const wchar_t* file)
{
	...
	{
		cs.Leave();
		SetLastError(DECODER_ERROR_INTERNAL_ERROR, L"WMCreateSyncReader error.");
		return false;
	}
	...
	if (hr != S_OK)
	{
		...
		cs.Leave();
		SetLastError(DECODER_ERROR_BAD_FORMAT, L"No file Output.");
		return false;
	}
...
int CWmaDecoder::GetChannelsCount()
{
	return dwChannels;
}
int CWmaDecoder::GetBitsPerSample()
{
	return dwBitsPerSample;
}
ULONG CWmaDecoder::GetSampleRate()
{
	return dwSampleRate;
}
...
DWORD CWmaDecoder::GetLengthSample()
{
	return 0;
}

DWORD CWmaDecoder::GetCurrentPositionSample()
{
...
double CWmaDecoder::GetLengthSecond()
{
	return (double)(liDuration.LowPart / 0.1e8);
}
double CWmaDecoder::SeekToSecond(double sec)
{
...
double CWmaDecoder::GetCurrentPositionSecond()
{
