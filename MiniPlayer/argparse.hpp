...
class argparser
{
  ...
  public:
    ...
    T get_argument(const std::string &name) const
    {
        ...
        if (named_arguments.size() > 0 && pos == named_arguments.cend())
        {
    ...
    std::string get_argument_string_raw(const std::string& name) const {
      auto pos = find_argument(name);
      if (pos == arguments.cend())
      {
        pos = find_named_argument(name);
      ...
      if (pos->type != type_string<std::string>())
      {
        std::cerr << "(get error) argument type mismatch: set '" << pos->type << "' but you try get with '"
          << type_string<std::string>() << "'" << std::endl;
        std::exit(-1);
      }
      return pos->value;
    }
